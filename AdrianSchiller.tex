...
\end{figure}
	...
	The final study, by Pretorius $et$ $al.$ ~\cite{Pretorius:2009:TAN:1632149.1632171}, created a Lego Mindstorm robot which would track its own position and heading from an evolved controller (Figure \ref{pRobot}). Because tracking a robot without sensory input is difficult, ER was applied to create a correlation between motor movement and location/orientation. The robot consists of two motorized wheels and has two blue and purple tracking markers on the top, as well as a light sensor. The markers and light sensor were used by an overhead camera to collect position and orientation information which was used to train the controller.
...
% Figure \ref{rewardsZone} compares the final and an earily candidate and their ability to maintain position over time.
